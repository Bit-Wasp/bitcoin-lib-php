...
class BIP32
{
    ...
    public static function CKD($master, $address_definition, $generated = array())
    {
        ...
        if ($previous['type'] == 'private') {
            ...
            $key_dec = $math->mod($math->add($Il_dec, $private_key_dec), $g->getOrder());
    ...
    public static function import($ext_key)
    {
        ...
        $key['depth'] = $math->hexDec(substr($hex, 8, 2));
    ...
    {
        $math = EccFactory::getAdapter();
        return $math->cmp($math->hexDec($hex), $math->hexDec('80000000')) == -1
            ? 0
            : 1;
    ...
    public static function check_valid_hmac_key($key)
    {
        ...
        // compare it to zero and the order of the curve
        $_equal_zero = $math->cmp($g_l, 0);
        $_GE_n = $math->cmp($g_l, $g->getOrder());
