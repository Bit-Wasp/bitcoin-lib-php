...
class BitcoinLibTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testSatoshiConversion() {
        ...
        foreach ($toSatoshi as $i => $test) {
            ...
            $this->assertEquals($satoshi, $this->bitcoin->toSatoshi($btc), "[{$i}] {$btc} => {$satoshi}");
            $this->assertTrue($satoshi === $this->bitcoin->toSatoshi($btc), "[{$i}] {$btc} => {$satoshi}");
        }
        ...
        foreach ($toBTC as $i => $test) {
            ...
            $this->assertEquals($btc, $this->bitcoin->toBTC($satoshi), "[{$i}] {$satoshi} => {$btc}");
            $this->assertTrue($btc === $this->bitcoin->toBTC($satoshi), "[{$i}] {$satoshi} => {$btc}");
        }
    }
}
